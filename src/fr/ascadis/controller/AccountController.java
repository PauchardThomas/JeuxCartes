...
public class AccountController implements Serializable {
    ...
    public String subscribe() {
        ...
        roles.add(this.roleDAO.find(2));
        this.utilisateur.setRoles(roles);
        // this.utilisateurDAO.save(this.utilisateur);
        this.utilisateurBean.saveUser(this.utilisateur);
    ...
    public String login() {
        ...
        Utilisateur myUtilisateur = this.utilisateurBean.findByLogin(this.utilisateur.getUsername(),
                this.utilisateur.getPassword());
        if (myUtilisateur != null) {
            Authentication myAuthentication = this.authenticationManager
                    .authenticate(new UsernamePasswordAuthenticationToken(this.utilisateur.getUsername(),
                            this.utilisateur.getPassword()));
            if (myAuthentication.isAuthenticated()) {
                SecurityContextHolder.getContext().setAuthentication(myAuthentication);
                myContext.getExternalContext().getSessionMap().put("SPRING_SECURITY_CONTEXT",
